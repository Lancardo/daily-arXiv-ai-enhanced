...
TL;DR: 提出了一种基于AI增强的广义偏振共路干涉测量法(GPCPI)，通过偏振解耦和深度神经网络实现宽带相位测量，相比现有技术提高了10倍的相位稳定性，并应用于单细胞色散成像和癌症细胞分类。


<details>
  <summary>Details</summary>
Motivation: 传统干涉测量技术虽然能提供高灵敏度的相位信息，但对偏振条件要求严格且易受外部扰动（如振动）影响，导致系统不稳定和噪声增加。需要一种更稳定、宽带且能实时追踪相位变化的干涉测量方法。

Method: 开发了广义偏振共路干涉测量法(GPCPI)，通过偏振解耦放宽传统共路干涉测量的偏振约束，结合深度神经网络自编码器分析相位轮廓的二阶导数映射来检测相位异常，采用ConvNeXt V2模型实现单次实时相位变化追踪。

Result: 相比最先进的干涉测量技术，相位稳定性提高了一个数量级，实现了宽带相位测量和高精度相位传感。成功应用于等离子体超表面相位传感和超光谱单细胞色散成像，能够区分正常皮肤细胞(CCD-32Sk)和癌细胞(COLO-829)，实现单细胞水平的稳健分类和疾病诊断。

Conclusion: GPCPI技术为宽带相位测量和单细胞色散成像提供了一种可靠、紧凑且稳定的解决方案，在计量学、分子诊断、药物发现和量子传感等领域具有广泛应用前景。
...
TL;DR: 该论文展示了基于蓝宝石上晶体硅的元表面，可将高斯光束转换为复杂光学陷阱阵列，包括用于原子捕获的暗区光学瓶状光束阵列。


<details>
  <summary>Details</summary>
Motivation: 传统空间光调制器(SLMs)虽然广泛用于生成陷阱阵列，但存在像素数量有限、动态噪声等问题。需要一种可扩展、紧凑且低噪声的解决方案来生成大规模复杂光学陷阱阵列。

Method: 使用CMOS兼容工艺制造蓝宝石上晶体硅(c-SOS)元表面，通过修改Gerchberg-Saxton算法来设计能在焦平面产生复杂振幅分布的元表面，优化陷阱阵列的均匀性。

Result: 成功制造并测量了c-SOS元表面，能够将高斯激光束转换为亮陷阱阵列、暗陷阱阵列以及亮/暗交替陷阱阵列，实现了三维瓶状光束作为暗陷阱。

Conclusion: c-SOS元表面提供了一种可扩展、紧凑且低噪声的光学陷阱生成方案，相比传统SLMs具有像素数量无限扩展、体积小、动态噪声低等优势，适用于大规模复杂光学陷阱阵列应用。
...
TL;DR: 提出基于物理信息神经网络（PINN）的端到端太赫兹三维全息超表面设计方法，相比传统迭代算法具有更高成像质量和更快计算速度。


<details>
  <summary>Details</summary>
Motivation: 传统全息超表面设计算法需要在超表面和目标平面之间多次迭代，导致计算时间过长。需要一种更高效的设计方法来实现高质量、实时的太赫兹三维全息显示技术。

Method: 采用物理信息神经网络（PINN）进行自监督训练，无需配对输入-标签数据集。训练后实现目标全息图案与超表面结构之间的端到端映射，同时考虑相位和振幅调制。

Result: PINN设计的超表面在单平面和三维多平面全息成像中均表现出比传统迭代算法更高的成像质量。在物理反直觉场景下也能找到近似解，推理过程通常不到1秒，远快于传统算法。

Conclusion: 这种端到端设计方法为实现高质量、实时、大规模太赫兹三维全息显示技术开辟了新途径，同时处理相位和振幅调制的能力使其在处理复杂物理场景时优于传统相位调制算法。
...
TL;DR: 首次展示四元数光学计算芯片，通过波分复用实现高维数据处理，在三维点云处理、RGB色彩变换和彩色图像识别等应用中相比电子计算具有更高计算保真度和更低计算负载。


<details>
  <summary>Details</summary>
Motivation: 传统实数和复数计算芯片在处理高维数据时存在根本性限制，而四元数能够直接处理三维和四维数据，为现代信号处理中的高维数据处理提供支持。

Method: 开发四元数光学计算芯片，利用波分复用技术通过多个光学波长通道同时处理高维数据，利用光的高并行性。

Result: 相比电子计算，QOCC实现了更高的计算保真度（均方根误差<0.035）和显著降低的计算负载（降低2/3），在三维点云处理、RGB色彩变换和四元数卷积神经网络彩色图像识别等应用中表现出色。

Conclusion: 四元数光学计算芯片克服了传统计算系统在高维数据处理中的限制，为下一代光学计算开辟了新途径。
...
TL;DR: 提出一种时空编码超表面系统，能够直接在电磁波空间能量分布上执行微积分运算，通过时变编码的谐波特性实现复杂任务处理。


<details>
  <summary>Details</summary>
Motivation: 随着超表面的快速发展和可编程超表面简化信息系统的需求增加，基于超表面的波计算成为一个有吸引力的研究方向。为了促进电磁空间中的数学运算，需要开发能够直接对电磁波空间能量分布进行微积分运算的系统。

Method: 提出时空编码超表面系统，利用时变编码诱导的谐波特性，灵活控制特定谐波下的元原子响应。开发用于电磁波微分和积分的时空编码序列，并设计能够独立且同时在同一入射电磁波上实现不同微积分运算的编码序列。通过可编程2位时空编码超表面进行概念验证实验。

Result: 理论、数值模拟和实验之间的一致性良好，证实了在电磁空间中执行微积分运算的可行性。时空编码超表面能够动态实时切换功能以适应不同的微积分任务。

Conclusion: 时空编码超表面系统在电磁波操纵、无线通信和信号处理方面具有广阔的应用前景，为电磁空间中的数学运算提供了有效解决方案。
...
TL;DR: 提出"混沌梳"架构，突破光学混沌带宽与并行通道数的限制，实现543.8GHz超宽带混沌和可编程并行通道，应用于太赫兹噪声源和256臂赌博机问题的超快光子决策


<details>
  <summary>Details</summary>
Motivation: 光学混沌在安全通信、激光雷达和强化学习中具有重要应用前景，但其可扩展性长期受限于带宽与并行混沌通道数量之间的固有权衡

Method: 采用可编程"混沌梳"架构，通过标准电光组件实现：将延迟反馈混沌激光器与连续波参考源外差混频，产生宽带混沌微波信号驱动级联电光梳，在所有梳齿上印刻混沌动力学并合并为超宽带混沌连续谱，结合频谱切片实现可编程并行通道提取

Result: 实现了单通道543.8GHz有效带宽的超宽带光学混沌；创建了超平坦太赫兹噪声源，过剩噪声比为52.99±2.85dB；在256臂赌博机问题中利用非相关并行混沌实现超快光子决策，获得0.86的有利幂律缩放指数

Conclusion: 该工作为可编程、可重构、应用就绪的光子混沌系统开辟了新途径，突破了传统光学混沌系统的可扩展性限制
...
TL;DR: 论文提出了两种全波方法（EIE和CEM）来分析开放非圆形旋磁波导的模态特性，无需近似即可计算传播常数，并验证了方法的准确性和计算性能。


<details>
  <summary>Details</summary>
Motivation: 将波导传播理论从传统的介电/各向同性材料扩展到旋磁材料，以支持当代波导结构的设计需求。

Method: 开发了两种独立的全波方法：扩展积分方程法（EIE）和切比雪夫展开法（CEM），均使用基于超势的柱面矢量波函数展开旋磁核中的电磁场。

Result: 两种方法都能准确计算非圆形旋磁波导的传播常数，与商业有限元求解器结果一致，并展示了在微波应用中计算铁氧体棒复杂传播常数的能力。

Conclusion: 该工作推进了开放波导传播理论，从介电/各向同性材料扩展到旋磁材料，为现代波导结构设计提供了有效工具。
...
TL;DR: 该论文提出了一个变分框架，用于分析光学腔中可移动镜面与经典电磁场的耦合动力学，涵盖非相对论和相对论运动机制。


<details>
  <summary>Details</summary>
Motivation: 可移动镜面的光学腔为研究辐射-物质相互作用和光控机械效应提供了重要平台，但在一致的理论框架中捕捉电磁场与镜面的耦合动力学仍然具有挑战性。

Method: 从受外部势作用的镜面运动方程出发，提出了镜面-辐射相互作用的变分公式，并引入了单模变分近似来捕捉耦合系统的关键动力学特征。

Result: 在镜面自由运动的特殊情况下，变分方法给出了精确解。该统一处理揭示了不同动力学机制之间的联系。

Conclusion: 该变分框架为分析从精密干涉测量到相对论辐射压力效应的广泛应用提供了理论基础。
...
TL;DR: 提出一种声学驱动的植入式激光二极管，通过MHz超声波无线供电，实现深层组织光声成像，突破传统表面照明的深度限制。


<details>
  <summary>Details</summary>
Motivation: 传统光声成像依赖外部光源，受光学衰减限制，成像深度仅数厘米。需要突破这一限制，实现对深层组织（如胃肠道等表面照明无法到达区域）的成像能力。

Method: 开发紧凑的声学供电装置，利用MHz超声波的弱衰减特性，通过组织无线传输声能，为植入的脉冲激光二极管供电。激光脉冲在目标介质内部产生光声信号，编码局部光学吸收信息。

Result: 成功通过12厘米厚的体模进行成像验证，展示了在临床相关深度实现光声成像的可行性。

Conclusion: 该声学供电的植入式光声成像方法为深层组织成像开辟了新途径，特别适用于表面照明无法到达的解剖区域。
...
TL;DR: 提出基于量子发射器的全光学深度学习架构，通过量子非线性激活实现超低功耗AI推理，功率预算降低7个数量级


<details>
  <summary>Details</summary>
Motivation: 深度神经网络快速扩展带来不可持续的功耗问题，而现有光学神经网络缺乏高效的光学非线性，限制了其能力

Method: 将量子发射器嵌入逆向设计的纳米光子结构中，利用其饱和性产生强非线性；采用物理感知训练，并建立非线性与网络表达能力之间的定量评估框架

Result: 实现了非线性分类和强化学习等复杂任务，量子激活在nW/μm²强度下工作，功率预算降低7个数量级；大语言模型所需光功率随模型规模亚线性增长，可实现瓦级运行

Conclusion: 量子纳米光子学为实现可持续AI推理提供了一条可行路径
...
TL;DR: 研究人员利用Janus粒子（半镀金介电微球）在流体环境中实现了可调谐的横向光力，仅通过旋转线偏振光方向即可控制粒子侧向运动。


<details>
  <summary>Details</summary>
Motivation: 横向光力通常局限于界面几何结构（如空气-水边界），研究人员希望探索在纯流体环境中实现可编程、偏振控制的横向光力操纵机制。

Method: 使用Janus粒子（介电微球半镀金）作为研究对象，通过调整线偏振光束的偏振角以及粒子尺寸和方向等参数，在流体环境中产生可调谐的横向光力。

Result: 实验观察到Janus粒子在水中仅通过旋转偏振方向即可实现完全可逆的横向推进，实验结果与理论预测高度一致，证明了偏振控制的可编程光学操纵机制。

Conclusion: 该研究建立了在流体环境中实现可编程、偏振控制光学操纵的新机制，对生物光子学、微流控和活性软物质系统具有重要应用前景。
...
TL;DR: 该论文系统研究了任意子PT对称性在波导晶格中的相变和拓扑保护特性，发现对称性未破缺时本征值幅角被限制在两个离散值，导致独特的振荡传播动力学，并揭示了拓扑边缘态受广义伪任意子厄米对称性保护。


<details>
  <summary>Details</summary>
Motivation: PT对称性和反PT对称性因其非厄米谱特性受到广泛关注，但它们在波导平台中的物理意义以及在扩展晶格系统中的拓扑特性尚未充分探索。任意子PT对称性作为更一般的框架，其物理含义和拓扑特征需要系统研究。

Method: 在波导晶格中系统研究任意子PT对称性系统的相变和拓扑保护。分析一维晶格能带结构，研究对称性破缺相变过程中的能隙闭合和重开现象，并探讨拓扑边缘态的保护机制。

Result: 在对称性未破缺区域，所有体态本征值的幅角被限制在两个离散值（相差π），导致独特的振荡传播动力学并伴随可控放大或耗散。一维晶格能带在相变过程中经历能隙闭合和重开，且在对称性未破缺区域，拓扑边缘态出现在体能隙中并受广义伪任意子厄米对称性保护。

Conclusion: 任意子PT对称性为非厄米波导系统提供了新的可调自由度，其中本征值幅角为信息编码提供了自然量。这项工作拓宽了广义非厄米对称性下拓扑保护的概念基础。
...
TL;DR: 该论文提出了一种受子对称性保护的拓扑紧致边缘态，这种态在晶格边界处仅局域在两个格点上，完全不向体区衰减，为拓扑光子学应用提供了新途径。


<details>
  <summary>Details</summary>
Motivation: 传统对称性保护拓扑相和子对称性保护拓扑态的边界态通常向体区指数衰减，无法实现极端局域化。本文旨在探索能否实现完全紧致（仅局域在边界两个格点）的拓扑边缘态，这对于紧凑型光子器件应用具有重要意义。

Method: 通过设计具有菱形类元胞的光子晶格结构，利用边界处的局域相消干涉实现紧致局域化，同时通过子对称性提供拓扑保护，用缠绕数等拓扑不变量表征拓扑性质。

Result: 在激光直写制备的光子晶格中实验观测到了紧致边缘态，这些态仅局域在边界两个格点上，不向体区衰减，并且在手征对称性和子对称性条件下对微扰表现出鲁棒性。

Conclusion: 子对称性保护可以实现极端局域化的拓扑紧致边缘态，为紧凑型波导、激光器和高灵敏度光子传感器等应用开辟了新途径，展示了子对称性在拓扑光子学中的重要潜力。
...
TL;DR: 开发了一种基于石墨烯等离子体极化子原子腔的单片探测器，在深亚波长尺度（2微米，1/60波长）下实现了室温、快速、偏振分辨的太赫兹检测。


<details>
  <summary>Details</summary>
Motivation: 室温、小型化、偏振分辨的太赫兹高速检测对于雷达、遥感、半导体检测中的高分辨率成像至关重要，也是大规模太赫兹焦平面阵列的关键。然而，在深亚波长尺度（<1/50波长）下的小型化仍然具有挑战性，因为弱的光-物质相互作用会降低响应度和偏振灵敏度。

Method: 提出石墨烯等离子体极化子原子腔单片探测器，集成石墨烯矩形PPAC阵列与异质金属接触。石墨烯同时作为吸收体和导体，简化了结构。利用等离子体极化子共振和光热电效应实现检测。

Result: 探测器在0.53-4.24 THz范围内实现了偏振敏感、频率选择性和快速的太赫兹检测，偏振比达93，响应度1007 V/W，噪声等效功率16 pW/Hz^0.5，比探测率2.9×10^7 Jones，响应时间230 ps。展示了偏振成像和无损半导体芯片检测应用。

Conclusion: 该工作突破了深亚波长尺度下太赫兹探测器性能下降的限制，在仅2微米通道长度下保持甚至增强了性能，推动了室温、紧凑、偏振敏感太赫兹技术的发展。
...
TL;DR: 提出一种集成液晶层的等离子体纳米激光器，通过电压控制实现主动光束偏转，单模器件偏转角度达±67°，双模器件支持两个波长同时调谐。
...
Method: 数值模拟等离子体纳米激光器集成电压控制液晶层，建立液晶分子在偏压下的取向模型，结合电磁仿真评估光学响应，并扩展到基于合并纳米孔阵列的双模纳米激光器。

Result: 单模PNL在870nm波长实现激光发射，电压依赖的远场偏转达±67°，光束发散角小于1°；双模器件在873nm和880nm两个波长同时实现电压依赖的角度调谐，无需改变腔体结构。

Conclusion: 液晶集成的等离子体纳米激光器作为电压控制可重构光源，在光互连、光束路由、自适应成像和多路复用通信中具有应用潜力。
...
TL;DR: 提出一种基于瞬态光栅傅里叶变换和虚拟锁相放大技术的实验方法，首次直接测量光生载流子的平均自由程，发现硅和砷化镓中热载流子的平均自由程可达微米级，比电学测量值大一个数量级以上。


<details>
  <summary>Details</summary>
Motivation: 光生载流子的平均自由程是材料设计和器件优化的关键参数，但目前许多材料的这一参数未知，且缺乏有效的实验测量方法。同时，光生高能热载流子的平均自由程是否与费米面附近或带边的局域平衡态载流子有显著差异仍不清楚。

Method: 基于瞬态光栅傅里叶变换概念，利用虚拟锁相放大技术，开发了一种测量光生载流子平均自由程的高效实验技术。该方法能够直接观察载流子在扩散和弹道运动之间的输运行为。

Result: 在硅膜和砷化镓量子阱中，光生热载流子的平均自由程可达微米尺度，比电学测量的值大一个数量级以上。该方法成功观测到了载流子从扩散到弹道运动的转变过程。

Conclusion: 该工作为表征工作状态下的光电器件提供了新思路，有望极大增强对光电或光子材料中光生载流子输运过程的理解，并为材料设计和器件优化提供重要参数。
...
TL;DR: 通过时间介电调制诱导的弹性和非弹性Floquet散射通道之间的相位控制干涉，实现纳米尺度网络中辐射热传输的可编程光子控制


<details>
  <summary>Details</summary>
Motivation: 开发一种可重构的纳米尺度光子热管理平台，实现热路由和逻辑操作

Method: 利用时间介电调制产生弹性和非弹性Floquet散射通道，通过相对调制相位控制这些通道之间的干涉，实现热流的方向控制和重新分布

Result: 实现了方向性热光子流和热分流，即使在热平衡状态下也能工作；调制幅度和频率可进一步调节能量流的增强、抑制和重新分布

Conclusion: 这种基于干涉的机制为纳米尺度可重构光子热管理提供了通用平台，可实现热路由和逻辑操作
...
TL;DR: 该论文展示了腔增强光声传感器可在低压下实现亚多普勒饱和光谱，同时保持高灵敏度，为计量学和基础物理研究提供了结合高灵敏度与高分辨率的中红外工具。


<details>
  <summary>Details</summary>
Motivation: 分子正成为计量学和基础物理研究的新基准，需要兼具高灵敏度和高分辨率的光谱技术。传统光声光谱主要关注高压痕量气体分析，而低压测量对于高分辨率光谱至关重要，但被认为与高效声波传播不兼容。

Method: 使用腔增强光声传感器，在毫瓦级中红外辐射下对低压痕量气体进行亚多普勒饱和光谱测量。同一装置也可在较高压力下运行，实现痕量气体检测。

Result: 成功在低压下实现了亚多普勒饱和光谱测量，同时在较高压力下实现了5ppb的检测灵敏度（仅需35微瓦激光功率）。

Conclusion: 该研究将光声技术的独特优势扩展到计量学和基础物理领域，为中红外光谱提供了兼具高灵敏度、高分辨率、成本效益和灵活性的工具。
...
TL;DR: 该研究建立了一个基于分子光力学的量子力学框架来分析IVR，并提出了两种泵浦-探测配置来优化IVR表征，展示了在单分子水平上检测IVR的可行性。


<details>
  <summary>Details</summary>
Motivation: 目前表面增强振动光谱技术虽然能够以单分子灵敏度研究分子振动，但尚未充分开发用于表征分子内振动再分布（IVR）。开发准确的IVR表征工具对化学研究具有重要意义。

Method: 建立基于分子光力学的量子力学框架来模拟IVR，分析两种泵浦-探测配置：红外激光泵浦和斯托克斯SERS泵浦，通过反斯托克斯SERS光谱检测耦合振动模式间的粒子数转移。

Result: 两种泵浦配置都在反斯托克斯SERS光谱中显示出耦合振动模式间粒子数转移的清晰特征，这些IVR特征在现有实验平台上可在单分子水平上检测到。

Conclusion: 该研究为利用表面增强振动光谱在单分子水平上表征IVR提供了理论框架和实验策略，填补了该领域的技术空白。
...
TL;DR: 非线性拓扑物理研究：全对全非局域相互作用SSH晶格中，非线性保持有效手征对称性，导致量子化非线性绕数和Berry相位，驱动拓扑相变，产生分数绕数和双周期Bloch振荡，与离散时间晶体相关。


<details>
  <summary>Details</summary>
Motivation: 探索拓扑与非线性的相互作用是现代物理学的核心挑战。研究旨在理解非线性如何影响拓扑性质，以及是否能在非线性系统中实现新的拓扑现象。

Method: 采用具有全对全非局域相互作用的合成Su-Schrieffer-Heeger晶格模型，发展Bogoliubov非线性绝热理论，分析非线性对拓扑性质的影响。

Result: 发现非线性保持有效手征对称性，导致量子化非线性绕数和Berry相位。非线性增加驱动拓扑相变序列，产生特征性燕尾带结构和带交换，导致分数绕数和双周期Bloch振荡。即使从拓扑平凡线性系统出发，非局域非线性也能诱导出具有分数绕数的涌现拓扑相。

Conclusion: 该研究建立了严格的非线性拓扑理论框架，为在合成量子平台中探索非线性拓扑现象及其应用开辟了新途径，特别是在具有里德堡相互作用的简并光学腔中可实现实验验证。
...
TL;DR: 该综述提出基于物理机制的单光子源分类框架，重点分析杂化钙钛矿量子点和压缩真空态在解决传统单光子源性能限制方面的潜力，为可扩展量子光子架构提供指导。


<details>
  <summary>Details</summary>
Motivation: 单光子源在量子通信、计算和计量中至关重要，但传统源在纯度、不可区分性和可调性方面存在性能限制，需要新的物理机制和材料平台来突破这些限制。

Method: 采用基于物理机制的分类框架，比较分析传统量子发射器和非线性光学过程，重点研究杂化有机-无机钙钛矿量子点的性能特征，并引入性能框架评估可扩展单光子源，探讨压缩真空态的潜在应用。

Result: 杂化钙钛矿量子点具有尺寸和组分可调的发射特性、窄线宽和室温工作能力，有望解决传统单光子源的关键限制；压缩真空态机制为高纯度、可复用的光子生成提供了超越传统预示方案的新途径。

Conclusion: 通过整合杂化钙钛矿量子点和压缩真空态概念，可以开发出可扩展的量子光子架构，为下一代高性能单光子源的发展指明方向。
...
TL;DR: 本文展示了一种紧凑、热稳定的平台，通过Yb:KGW激光器在40-600 kHz高重复频率下同时产生宽带太赫兹和紫外脉冲，解决了高平均功率非线性转换中的热效应和材料损伤问题。


<details>
  <summary>Details</summary>
Motivation: 将飞秒太赫兹和紫外光源扩展到高重复频率对于高通量超快光谱和成像应用至关重要，但高平均功率下的高效生成受到热效应、相位匹配约束和材料损伤的限制。

Method: 使用Yb:KGW激光器在40-600 kHz工作，通过化学计量MgO:LiNbO₃中的光整流产生太赫兹辐射（采用线聚焦几何结构），并通过β-硼酸钡中的级联四次谐波产生产生257.5 nm紫外脉冲。

Result: 太赫兹产生：55-92 nJ单周期脉冲，峰值电场37-90 kV/cm；紫外产生：在40 kHz时转换效率超过10%，稳定运行至600 kHz；通过横向平移晶体内的产生区域，可在中心频率、带宽和场振幅之间实现可调控制。

Conclusion: 该研究展示了一个紧凑、热稳定的高平均功率非线性转换平台，可直接应用于下一代高重复频率超快电子衍射和光谱系统。
