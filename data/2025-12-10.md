...
Motivation: 研究周期性结构（特别是局域共振超材料链）中非线性波传播现象，传统方法在处理欠阻尼系统和非线性相互作用时存在局限性，需要发展更精确的高阶近似方法。

Method: 提出两种策略：1) CNF-BP：先对运动方程应用Bloch假设，再应用参数化方法的复正规形式非线性坐标变换；2) CNF-PN：先应用参数化方法将物理坐标变换为正规坐标，再对正规坐标施加Bloch波假设。

Result: 在保守情况下，CNF-PN策略在捕捉复杂波传播现象方面表现更优，能处理非基频谐波和宿主振子间的非线性相互作用；在欠阻尼系统中，CNF-PN经过严格验证，与数值技术、多尺度法和环形链结构直接数值积分结果一致。

Conclusion: CNF-PN策略比CNF-BP策略更有效，特别适用于处理周期性结构中复杂的非线性波传播问题，为欠阻尼超材料链的色散关系分析提供了更精确的高阶近似方法。
...
TL;DR: 提出一种测量近共振热原子蒸气中群速度色散的新方法，使用连续波激光器配合弱相位调制和慢速光电探测器，通过扫描调制频率观察透射调制对比度的振荡来直接测量GVD。


<details>
  <summary>Details</summary>
Motivation: 在近共振热原子蒸气中测量群速度色散（GVD）很困难，因为吸收、脉冲畸变和非线性效应会严重影响探针信号并降低信噪比，传统脉冲展宽或干涉测量技术难以应用。

Method: 采用连续波激光器配合弱相位调制和慢速光电探测器，受流体光中布拉格光谱启发，通过扫描调制频率，观察红移和蓝移边带积累的不同色散相移导致的透射调制对比度振荡，对比度消失的特定频率直接给出GVD。

Result: 将该技术应用于热铷蒸气，在D2线宽泛失谐范围内不同温度下观察到了GVD的强烈频率依赖性。

Conclusion: 该方法提供了一种简单有效的测量近共振热原子蒸气中群速度色散的技术，克服了传统方法的局限性，成功应用于铷蒸气并揭示了GVD的频率依赖性。
...
TL;DR: 研究人员使用周期性极化薄膜钽酸锂波导实现了瓦级连续波二次谐波产生，克服了传统钽酸锂功率限制


<details>
  <summary>Details</summary>
Motivation: 传统二次谐波产生技术存在限制：体晶体效率低、波导需要长长度、钽酸锂损伤阈值低。需要开发高功率、紧凑的集成非线性光学器件

Method: 采用7毫米长的周期性极化薄膜钽酸锂波导，优化电极几何形状和极化条件，克服钽酸锂高矫顽场的挑战

Result: 在4.5瓦泵浦功率下，产生超过1瓦的二次谐波功率，离芯片输出功率超过0.5瓦（775纳米），实现了瓦级连续波操作

Conclusion: 周期性极化薄膜钽酸锂波导为高功率集成非线性光学应用提供了有前景的平台，适用于集成激光器、量子光子学、光学时钟和频率计量等领域
...
TL;DR: 金属透镜优化的全色成像在计算机视觉任务中表现优异，保持空间频率信息是光学神经网络性能的关键因素。


<details>
  <summary>Details</summary>
Motivation: 静态超光学编码器虽然能耗低，但泛化能力不足限制了其广泛应用。研究旨在探索金属透镜在通用计算机视觉任务中的效用。

Method: 1) 优化用于全色成像的金属透镜，与高端传感器受限光学系统比较；2) 设计端到端单孔径超表面用于ImageNet分类；3) 分析调制传递函数(MTF)平衡特性。

Result: 优化后的金属透镜在图像分类准确率上与高端光学系统相当，且在不同传感器像素尺寸下均优于双曲面金属透镜。优化超表面倾向于平衡各波长的MTF。

Conclusion: 空间频率域信息的保持是光学神经网络性能的关键可解释因素，为设计高性能ONN和超光学编码器提供了实践指导。
...
TL;DR: 提出基于光子数分辨双缝干涉的自参考量子态层析方法，无需本地振荡器，利用两个相同量子场副本实现完美时空模式匹配。


<details>
  <summary>Details</summary>
Motivation: 传统量子态层析需要已知参考态或模式匹配的本地振荡器，这在实际应用中存在困难。本文旨在开发一种自参考方法，避免这些限制，同时借鉴经典超快光学中的成熟技术。

Method: 使用两个相同的未知量子场副本照射横向位移的双缝，在远场通过横向位置x测量相对缝相位φ(x)。N光子事件将检测到的量子场投影到态|N;φ(x)⟩，得到的分布P(N,φ)是量子版本的FROG迹线。

Result: 该方法可以从二项式加权的自干涉中重建Fock空间波函数或密度矩阵，无需已知或模式匹配的参考，且与市售光子数分辨相机兼容。

Conclusion: 该方法概念简单、自动模式匹配，通过FROG类比可将成熟的超快光学方法（如混合态、叠层成像和矢量扩展）直接转移到量子光学中，为量子光子态层析提供了通用途径。
...
TL;DR: 提出一种基于注射器装载碳纳米管溶液与光纤尖端轴向对齐的新方法，利用激光光镊在水基CNT溶液中沉积纳米管到光纤横截面上，实现高限制、保护和相互作用的CNT沉积层。


<details>
  <summary>Details</summary>
Motivation: 开发一种更高效、可控的碳纳米管沉积方法，为工业光纤激光器和生物医学光声设备的实时监测提供有前景的替代方案。

Method: 使用注射器装载碳纳米管溶液，使其与光纤尖端轴向对齐。通过激光在水基CNT溶液中产生光镊效应，将纳米管沉积到光纤横截面上。调整参数可获得不同厚度的两层CNT沉积层。

Result: 成功实现了在光纤上沉积两层不同厚度的碳纳米管层。该方法比常用的烧杯方法使用更小的溶液体积，提供了纳米管、激光和光纤之间的高限制、保护和相互作用。

Conclusion: 该方法为实时监测应用提供了有前景的替代方案，对工业光纤激光器和生物医学光声设备的发展具有重要意义。
...
TL;DR: 提出了一种用于模拟无序量子发射体集体辐射的高效计算框架，通过累积展开方法将计算复杂度从指数级降低到多项式级，应用于金刚石中硅空位中心，揭示了集体行为的两个特征。


<details>
  <summary>Details</summary>
Motivation: 传统模拟无序量子发射体集体辐射的方法计算复杂度呈指数增长，难以处理大量发射体。需要开发高效的计算框架来模拟包含空间无序和均匀展宽等现实因素的系统，以预测可与实验直接比较的统计结果。

Method: 采用累积展开方法，将计算复杂度从指数级降低到多项式级，实现了对大量实现进行蒙特卡洛采样。将该框架应用于金刚石中的硅空位中心，考虑了空间无序和均匀展宽，对数百个SiV⁻团簇进行统计平均。

Result: 发现两个集体行为特征：1) 完全反转团簇的动力学显示超辐射发射需要足够多的发射体和高量子效率，且强近场偶极-偶极相互作用显著抑制了超辐射爆发；2) 连续波激发下的光致发光激发谱显示相互作用引起的共振峰分布展宽，相应的态密度呈现非零偏度。

Conclusion: 该框架通过纳入发射体团簇中的现实非均匀性，能够预测无序系综的统计特性，可直接与实验比较。该方法可推广到原子、分子和量子点等其他发射体类型，为分析现实量子系统中的集体行为提供了实用工具。
...
TL;DR: 该论文研究了vdW MoOCl2在紫外到可见光波段的介电张量特性，发现了新的双曲性窗口，并提出了基于扭曲螺旋异质结构的圆偏振器设计。


<details>
  <summary>Details</summary>
Motivation: 范德华晶体因其内在各向异性在纳米光子学中具有独特优势。MoOCl2作为具有面内双轴各向异性的vdW氧氯化物，其光学性质尚未充分探索，特别是在紫外到可见光波段。研究其介电张量特性和潜在的双曲性窗口，有望为纳米光子学应用提供新平台。

Method: 通过精确测量vdW MoOCl2在紫外(UV)到可见光(Vis)光谱区域（部分覆盖近红外NIR）的介电张量分量，分析其高度各向异性的光学常数。重点关注在紫外光谱区域出现的双曲性窗口，并提出基于扭曲螺旋vdW MoOCl2异质结构的纳米尺度手性保持可见光圆偏振器设计方法。

Result: 研究发现vdW MoOCl2在紫外光谱区域存在另一个双曲性窗口，这可能解锁丰富的光-物质相互作用效应。同时，基于扭曲螺旋异质结构的圆偏振器设计显示出良好的手性保持特性。

Conclusion: vdW MoOCl2不仅为双曲纳米光子学应用提供了有前景的平台，也为手性纳米光子学应用开辟了新途径。该材料在紫外到可见光波段的独特光学性质使其在纳米光子器件设计中具有重要价值。
...
TL;DR: 首次实验演示了碳纳米管钻孔光纤形成的激光诱导涡旋冲击波，通过高功率激光在光纤中产生超音速冲击波，在光纤端面形成斐波那契螺旋结构的钻孔


<details>
  <summary>Details</summary>
Motivation: 探索激光控制纳米粒子沉积和涡旋器件制造的新途径，为光纤通信、传感器、高功率脉冲光纤激光器和生物医学超声神经递质等领域开发新型全光纤涡旋空间相位调制器

Method: 将标准单模光纤插入装有碳纳米管-甲醇溶液的注射器中，注入高功率激光5-20分钟，利用碳纳米管溶液热膨胀产生的涡旋声流在光纤端面钻孔

Result: 实现了超音速冲击波（5742 m/s）和高压力（6.7 GPa），在光纤核心形成最大深度5微米的斐波那契螺旋钻孔，沉积了纳米级碳纳米管-二氧化硅层

Conclusion: 该方法为激光控制纳米粒子沉积和光纤端面涡旋器件制造开辟了新途径，在光学通信、光纤传感器、高功率激光器和生物医学应用方面具有广阔前景
...
TL;DR: 利用高折射率介电纳米结构调控Eu(TTA)3配合物的电偶极和磁偶极发射，通过硅二聚体结构实现偏振依赖的发射增强。


<details>
  <summary>Details</summary>
Motivation: 探索利用介电纳米结构的Mie共振来调控稀土离子（Eu3+）的电偶极和磁偶极发射，特别是通过工程化纳米结构实现对不同偶极跃迁的选择性调控。

Method: 采用有限元方法模拟优化样品参数，通过两步电子束光刻工艺制备混合纳米结构，将Eu3+掺杂的电子束抗蚀剂精确置于硅二聚体间隙中心。

Result: 硅二聚体结构在间隙区域产生偏振依赖的电场和磁场增强，分别调控Eu3+在610nm（电偶极）和590nm（磁偶极）的发射，实现了对两种不同偶极跃迁的选择性调控。

Conclusion: 介电纳米结构能够选择性调控稀土离子的电偶极和磁偶极发射，展示了磁光相互作用作为新型自由度在纳米光子学中的潜力。
...
TL;DR: PyMieDiff是一个完全可微分、GPU兼容的PyTorch实现，用于计算核壳粒子的Mie散射，支持梯度优化和物理信息神经网络集成。


<details>
  <summary>Details</summary>
Motivation: 随着机器学习能力的发展，需要端到端可微分的散射计算框架。传统Mie散射计算不可微分，限制了与机器学习方法的集成。

Method: 开发了PyMieDiff库，提供原生可微分的球贝塞尔和汉克尔函数，向量化计算Mie系数，支持计算效率、角散射和近场分布。所有输入表示为张量，可与梯度优化和物理信息神经网络无缝集成。

Result: 成功实现了完全可微分、GPU兼容的Mie散射计算框架，支持核壳粒子，可与TorchGDM结合进行端到端可微分的多粒子散射模拟。

Conclusion: PyMieDiff为散射计算提供了强大的可微分工具，促进了机器学习在光学、纳米技术和大气科学等领域的应用，代码已开源。
...
TL;DR: 开发了一个基于多重散射理论、兼容自动微分的逆设计框架，适用于周期和非周期散射体阵列，能够高效计算几何和位置参数的精确梯度。


<details>
  <summary>Details</summary>
Motivation: 随着亚10纳米制造技术的发展，设计空间急剧扩大，对逆设计的需求增加。现有全波求解器虽然可用于逆设计，但缺乏专门针对多重散射模拟的专用工具。

Method: 提出了一个兼容自动微分的多重散射框架，使用球体作为基本构建块，但可扩展到任意形状散射体（只要T矩阵通过可微分全波麦克斯韦求解器计算）。框架能同时计算多个参数的梯度，适合中等维度问题。

Result: 通过两个概念验证示例展示了方法的多样性：1）优化有限散射体簇实现高前向-后向散射比，并展示了实验可行性；2）设计每个单元包含多个散射体的超表面，最大化正交线偏振入射光的反射对比度。

Conclusion: 该框架填补了多重散射逆设计工具的空白，能够高效处理周期和非周期结构，为光子学逆设计提供了灵活且强大的工具，已开源供社区使用。
...
TL;DR: 该论文研究了非厄米系统中时间界面的拓扑效应，发现反射和折射系数呈现拓扑编织结构，建立了时间界面的体边对应关系，并提出了探测拓扑相变的动力学方法。


<details>
  <summary>Details</summary>
Motivation: 非厄米系统展现出独特的拓扑现象，但在时间界面（系统参数突变）上的表现尚未被充分探索。研究旨在揭示非厄米系统中时间界面的拓扑效应及其与拓扑不变量之间的关系。

Method: 引入具有手性对称性的哈密顿量，研究一维非厄米系统中时间界面的反射和折射现象。通过分析反射和折射系数的拓扑编织结构，建立与拓扑不变量差异的直接联系，并提出基于本征态几何相似性的动力学探测方法。

Result: 发现反射和折射系数呈现拓扑编织结构，该结构由界面两侧拓扑不变量的差异直接决定，从而建立了非厄米系统中时间界面的体边对应关系。提出的动力学探测方法能够有效检测拓扑相变。

Conclusion: 该研究建立了拓扑编织与非厄米系统中时间界面的非互易动力学之间的基本联系，为探索时变非厄米系统中的相变检测和非互易现象提供了平台。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于微谐振腔的电场传感技术需要可调谐低噪声激光器，成本高、系统庞大，限制了工业应用。需要探索使用低成本固定频率激光器的可能性。

Method: 采用低成本固定频率半导体激光器作为泵浦激光，利用铌酸锂微谐振腔的电光效应将谐振模式频率锁定到激光器，同时利用同一效应实现射频电场传感。

Result: 实验表明：1）电场测量分辨率在信号频率超出光学共振带宽时仍能保持；2）只要激光频率噪声是系统主要噪声源，信噪比不随耦合条件变化；3）在微谐振腔压电共振频率处观察到窄带电光灵敏度增强，在4MHz附近分辨率提升约3倍。

Conclusion: 该工作通过研究带宽限制推进了光子共振电场测量技术，为在低成本激光器中实现高分辨率传感应用开辟了道路。
...
TL;DR: 双波长傅里叶叠层形貌术突破单波长λ/2高度范围限制，通过合成波长实现更大无模糊测量范围，保持横向分辨率，适用于半导体计量。


<details>
  <summary>Details</summary>
Motivation: 单波长傅里叶叠层形貌术存在λ/2的高度范围限制，这限制了其在具有较大高度变化样品上的应用。需要一种方法在保持高横向分辨率的同时扩展测量范围。

Method: 使用双波长照明重建复振幅场，利用相位差产生合成波长λ_s。采用噪声鲁棒的包裹数搜索选择像素级整数对(k1,k2)，通过全局优化结合环形TV正则化和软约束提高稳定性并保持高度不连续性。

Result: 方法成功扩展了无模糊高度范围至λ_s/2，在单波长FPT出现相位包裹的情况下仍能准确恢复高度。通过散射模型仿真和结构化硅样品实验验证了有效性。

Conclusion: 双波长FPT在保持高横向分辨率的同时显著扩展了高度测量范围，通过分析纵横比和相位调制传递函数可预测重建质量，适用于半导体和工业计量应用。
...
TL;DR: 提出基于准连续束缚态(q-BIC)的时域耦合模理论模型，用于描述非对角手性双各向异性，在强电磁模式耦合下实现类拉比分裂和双波段非对称反射/吸收。


<details>
  <summary>Details</summary>
Motivation: 电磁模式耦合在纳米光子学中至关重要，它导致双各向异性现象（电响应和磁响应相互关联）。现有理论需要更简洁通用的框架来描述非对角手性双各向异性，特别是在强耦合区域。

Method: 开发了简单的时域耦合模理论模型，利用准连续束缚态(q-BIC)控制具有相反对称性的模式杂化，在强电磁模式耦合下实现类拉比分裂。

Result: 理论预测了双波段非对称反射和吸收的出现，以及如何实现最大方向吸收差异。全波仿真验证了理论预测，并在光学频率的深亚波长超表面上展示了非常强的互易双各向异性。

Conclusion: 该工作为模式相互作用提供了清晰的物理图像，为理解和设计双各向异性介电超表面（包括传统区域和强耦合区域）提供了紧凑的理论框架。
