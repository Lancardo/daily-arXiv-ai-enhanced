...
### [1] [Freely controllable single-optical-frequency comb for highly sensitive cavity ring-down spectroscopy](https://arxiv.org/abs/2512.09159)
*Norihiko Nishizawa,Shotaro Kitajima,Ningwu Liu,Ryohei Terabayashi,Daiki Hashimoto,Hisashi Abe,Hideki Tomita*

Main category: physics.optics

TL;DR: 开发了一种基于光谱峰值现象的可控光频梳源，通过将梳齿能量集中在甲烷吸收光谱重叠的模式上，实现了高灵敏度的腔衰荡光谱测量，灵敏度比先前报道的梳基CRDS高两个数量级。


<details>
  <summary>Details</summary>
Motivation: 传统直接梳光谱中，光能量分散在大量梳齿模式上，每个梳齿的光能量极小，限制了高灵敏度光谱测量的能力。需要将光能量集中在与吸收光谱重叠的梳齿上，以提高测量灵敏度。

Method: 开发了基于光谱峰值现象的可控光频梳源，利用甲烷气体池和非线性环形镜将重叠甲烷吸收光谱的梳齿转换为背景抑制的光谱峰值。使用光纤拉曼放大器进行相干保持功率放大，通过新型超高分辨率光谱滤波器筛选单梳齿模式。最后使用高精细度光学腔进行腔衰荡光谱测量。

Result: 单梳齿最大光功率超过10 mW，测量了甲烷2v_3波段的吸收光谱，灵敏度达到4.2×10^(-11) cm^(-1)，比先前报道的梳基CRDS高两个数量级，残差仅为0.29%，表明光谱仪具有高精度。

Conclusion: 该方法可将光能量集中在特定吸收光谱的梳齿上，实现了高灵敏度、高分辨率的光谱测量，可扩展到其他波长范围，对分子光谱分析具有重要应用价值。
...
### [2] [Ionic Disorder-Mediated Exfoliation and Optical Birefringence in a Non-van der Waals Oxide](https://arxiv.org/abs/2512.09248)
...
TL;DR: Na2Zn2TeO6 (NZTO)是一种可剥离的非范德华层状光学材料，具有强离子导电性和光学各向异性，为离子光子学提供了新平台。


<details>
  <summary>Details</summary>
Motivation: 扩展二维光子学材料库至非范德华层状系统，以获得更好的环境稳定性和新型功能（如强离子导电性），但这些材料的剥离一直具有挑战性。

Method: 1. 将Na2Zn2TeO6 (NZTO)确立为可剥离的非范德华光学材料；2. 利用其高度无序、可移动的Na+层间实现机械剥离至纳米厚度；3. 通过光谱椭偏仪进行光学表征；4. 通过温度分辨拉曼光谱研究晶格动力学。

Result: 1. NZTO可剥离至约4纳米厚度；2. 具有宽禁带介电特性和显著的光学双折射（Δn约0.25）；3. [Zn2TeO6]2-框架保持刚性，与高离子迁移率基本解耦；4. 确立了离子传输与光学控制融合的新范式。

Conclusion: NZTO为稳健、各向异性的介电光子学提供了有前景的平台，同时开辟了离子传输与光学控制融合的新途径，即"离子光子学"这一新兴范式。
...
### [3] [Scalable van der Waals Photonics: High-Refractive-Index Gallium Sulfide Films with Single-Crystal Optical Properties](https://arxiv.org/abs/2512.09255)
...
TL;DR: 本文展示了通过原子层沉积生长的硫化镓作为大规模范德华介电平台，在保持高光学性能的同时解决了可制造性问题，为可见光谱光子学中的各向异性材料应用提供了可行路径。


<details>
  <summary>Details</summary>
Motivation: 将高折射率介电材料集成到可扩展光子架构中是推进集成电路和增强现实显示器的关键。范德华材料具有优异的光学特性，但机械剥离法制备的薄片面积小、厚度不可控，限制了其实际应用。

Method: 采用原子层沉积技术生长硫化镓薄膜，通过光学和结构表征与原始单晶进行基准测试，验证其光学常数和性能，并利用其保留的面外各向异性特性演示在密集集成波导中的应用。

Result: ALD生长的GaS光学常数与单晶对应物几乎无法区分，且保留了面外各向异性，在密集集成波导中比传统可扩展高折射率平台能更有效地抑制串扰。

Conclusion: ALD-GaS克服了高光学性能与可制造性之间的权衡，为在可见光谱光子学中实现各向异性范德华材料提供了技术可行的途径。
...
### [4] [389.3-Tb/s 1017-km C-band Transmission over Field-Installed 12-Coupled-Core Fiber Cable with >12-Tb/s Spatial MIMO Channels](https://arxiv.org/abs/2512.09344)
...
TL;DR: 实现了4.65-THz WDM/SDM传输，在标准包层直径的12芯耦合光纤上传输140-Gbaud PS-QAM信号，创造了0.455 Pb/s的现场耦合芯容量记录，并展示了0.389 Pb/s超1000公里传输


<details>
  <summary>Details</summary>
Motivation: 为了在现有光纤基础设施上实现更高的传输容量，特别是在现场环境中突破容量限制，满足日益增长的数据传输需求

Method: 采用波分复用(WDM)和空分复用(SDM)技术，在标准包层直径的12芯耦合光纤上传输140-Gbaud PS-QAM信号，利用空间MIMO信道处理技术

Result: 实现了4.65-THz带宽传输，创造了0.455 Pb/s的现场耦合芯容量记录，并成功实现了0.389 Pb/s的超1000公里传输，单波长净比特率超过12 Tb/s

Conclusion: 该研究证明了在现有光纤基础设施上实现超高容量传输的可行性，为未来光通信系统的发展提供了重要技术支撑
...
### [5] [133-Tbps 1040-km (13$\times$80 km) Lumped-Amplified Transmission Over 22 THz in S-to-U-Band Using Hybrid Multiband Repeater with PPLN-Based Optical Parametric Amplifiers and EDFAs](https://arxiv.org/abs/2512.09347)
...
<details>
  <summary>Details</summary>
Motivation: 为了在长距离传输中实现更高的频谱效率和传输容量，需要扩展传输波段并开发新型放大器技术来支持超宽带信号传输

Method: 采用PPLN（周期性极化铌酸锂）光学参量放大器和EDFA（掺铒光纤放大器）组成的集总中继器，在S到U波段进行四波段传输，通过80公里SMF链路，考虑ISRS（受激拉曼散射）效应设计25.5-dBm光纤发射功率

Result: 在1040公里传输距离上实现了133.06 Tbps的净比特率，验证了22.05-THz四波段长距离传输系统的可行性

Conclusion: PPLN光学参量放大器与EDFA结合的集总中继器方案能够有效支持超宽带长距离传输，为实现更高容量的光纤通信系统提供了有前景的技术路径
...
### [6] [Twisted light from topological chiral exceptional points in a nanolaser array](https://arxiv.org/abs/2512.09388)
*Kaiwen Ji,Melissa Hedir,Qi Zhong,Ramy El-Ganainy,Alejandro M. Yacomotti,Li Ge*

Main category: physics.optics

TL;DR: 在InP基光子晶体膜上实现环形排列的轨道角动量纳米激光器阵列，通过非厄米扩展的Rice-Mele模型和PT对称泵浦方案，实现了对称保护的零模和手性异常点，产生具有可控角动量的结构化激光输出。


<details>
  <summary>Details</summary>
Motivation: 开发一种紧凑、可扩展的相位锁定激光器阵列，能够产生可控的轨道角动量输出，为集成光子平台上的结构化光生成开辟新途径。

Method: 在InP基光子晶体膜上设计环形排列的纳米激光器阵列，实现非厄米扩展的Rice-Mele模型，具有交替耦合强度和虚数在位失谐。采用PT对称泵浦方案调节增益/损耗对比度，达到手性异常点。

Result: 实验演示了对称保护的零模，相邻纳米激光器间具有均匀的π/2相移，形成阵列周围的相干相位缠绕。在异常点处实现单向能量流动，产生携带非零轨道角动量的远场发射。

Conclusion: 该研究展示了一种稳健且可扩展的策略，用于设计具有可控角动量输出的紧凑型相位锁定激光器阵列，为集成光子平台上的结构化光生成开辟了新途径。
...
### [7] [Photophoretic Trapping: Fundamentals, Advances and Future Directions](https://arxiv.org/abs/2512.09401)
*Anita Pahi,Kirty Ranjan Sahoo,Souvik Sil,Ayan Banerjee*

Main category: physics.optics

TL;DR: 本文综述了光泳力在粒子操控中的应用，重点介绍了其物理原理、实验配置、动力学现象及当前与新兴应用领域。


<details>
  <summary>Details</summary>
Motivation: 光泳力比辐射压强几个数量级，能在极低光强下实现粒子捕获，为气溶胶科学、自由空间3D体积显示乃至太空轻载荷部署等应用开辟了新途径。本文旨在系统统一光泳捕获的理论、实验和应用方面，推动这一快速发展领域的研究。

Method: 本文采用综述方法，全面解释光泳力的基础物理原理，分析实现稳定三维操控的实验配置，详细探讨控制光泳力大小和行为的物理参数，并讨论相关的动力学现象。

Result: 系统梳理了光泳捕获的理论框架、实验技术和应用前景，为理解光泳力如何实现吸收粒子的稳定三维操控提供了全面指导，并指出了当前和新兴的应用方向。

Conclusion: 光泳捕获是一个快速发展的跨学科领域，本文通过系统整合理论、实验和应用方面，旨在加强和推进该领域的研究，为未来的科学探索和技术创新提供基础。
...
### [8] [A power-in-bucket model enabled designs of nanostructure-enhanced waveguides for highly efficient wide-angle light couplings](https://arxiv.org/abs/2512.09405)
*Wenbo Luo,Yitong Gu,Jianwei Wang,Fei Yu,Chunlei Yu,Lili Hu,Zhichao Ruan,Ning Wang*

Main category: physics.optics

TL;DR: 提出功率桶模型分析结构修饰波导的耦合行为，验证环形光栅可显著提升大角度入射下的耦合效率


<details>
  <summary>Details</summary>
Motivation: 现有理论主要关注裸波导配置，缺乏对结构辅助耦合的全面考虑，需要新模型来探索结构修饰波导的耦合行为

Method: 引入功率桶模型，分析高斯光束和平板波两种代表性耦合场景，通过PIB优化光束-光纤参数

Result: PIB计算的耦合系数增强与实验值吻合良好，特别适用于多模光纤大角度照明；在37度入射角下，相同环形光栅可使η从0.3320提升至0.5102

Conclusion: PIB模型为光栅辅助光耦合机制提供了有用解释，有助于设计结构增强型探头用于广角宽带光收集应用
...
### [9] [Coherent Tunneling by Adiabatic Passage in Silicon Nitride based Integrated Waveguide Structures](https://arxiv.org/abs/2512.09436)
*Olga Borovkova,Valery Lobanov,Junqiu Liu,Dmitry Chermoshentsev,Igor Bilenko*

Main category: physics.optics

TL;DR: 本文展示了基于相干隧穿绝热通道(CTAP)的Si3N4平面波导结构在1.55μm通信波段的高效光路由和传输能力，提出了混合Si3N4-Si-Si3N4波导结构以提升性能。


<details>
  <summary>Details</summary>
Motivation: 硅氮化物光子集成电路已成为成熟平台，但需要探索更高效的光路由和传输机制。研究旨在利用CTAP原理在1.55μm通信波段实现高效光控制。

Method: 采用CMOS兼容技术制造平面波导和定向耦合器，研究高限制和低限制硅氮化物CTAP结构，并提出混合Si3N4-Si-Si3N4波导结构。

Result: 证明了CTAP结构的高效光路由能力，揭示了限制光控制效率的机制，确定了可实现的参数范围，混合结构相比单一材料结构展现出更高的效率和灵活性。

Conclusion: CTAP为硅氮化物光子集成电路提供了高效的光路由解决方案，混合波导结构进一步提升了性能，为光通信应用开辟了新途径。
...
### [10] [Casimir radiation with Weyl semimetals](https://arxiv.org/abs/2512.09455)
*Yang Hu,Xiaohu Wu,Haotuo Liu,Mauro Antezza,Xiuquan Huang*

Main category: physics.optics

TL;DR: 研究非互易Weyl半金属纳米颗粒的卡西米尔辐射，发现其热通量比简并模式纳米颗粒高27倍，为纳米尺度能量转换提供新方法。


<details>
  <summary>Details</summary>
Motivation: 卡西米尔摩擦扭矩作用于旋转纳米颗粒时，机械能可转化为热能（卡西米尔辐射），这对纳米机电系统的热性能有重要影响。研究非互易Weyl半金属纳米颗粒的卡西米尔辐射效应。

Method: 研究悬浮在平板上的非互易Weyl半金属纳米颗粒的卡西米尔辐射。通过分析电磁局域态密度在近场中的耦合与解耦机制，阐明其物理原理。

Result: 非互易Weyl半金属纳米颗粒的辐射热通量比简并模式纳米颗粒高27倍。Weyl半金属纳米颗粒的三重局域等离子体模式分裂为具有强旋磁响应的局域圆模式，为卡西米尔辐射开辟了新通道。

Conclusion: 这项工作为纳米颗粒系统中的纳米尺度能量转换提供了新方法，利用非互易Weyl半金属纳米颗粒的强旋磁响应特性显著增强卡西米尔辐射效应。
...
### [11] [High-resolution broadband characterization of resonance dispersion in an optical microresonator](https://arxiv.org/abs/2512.09475)
...
TL;DR: 提出了一种简单且高精度的测量硅氮化物微谐振器自由光谱范围的方法，利用0.4 MHz分辨率波长计在5 THz带宽内测量，可提取二阶和三阶自由光谱范围扩展系数。


<details>
  <summary>Details</summary>
Motivation: 准确了解光学微谐振器的不均匀自由光谱范围对于理解和控制Kerr频率梳动力学至关重要，因为它直接反映了群速度色散特性。

Method: 采用全光纤即插即用实验装置，利用0.4 MHz分辨率的波长计，通过仔细分析每个谐振的光谱位置，在5 THz带宽内测量硅氮化物微谐振器的自由光谱范围。

Result: 成功测量了硅氮化物微谐振器在5 THz带宽内的自由光谱范围，并提取了二阶和三阶自由光谱范围扩展系数。

Conclusion: 该方法为集成光子电路中的色散表征提供了一个稳健且易于使用的工具，为下一代Kerr梳源和量子光子技术铺平了道路。
...
### [12] [Four-wave Mixing Mediated Synchronization of Localized Polariton Condensates](https://arxiv.org/abs/2512.09531)
*Alexey V Yulin*

Main category: physics.optics

TL;DR: 通过四波混频产生自由传播的极化激元，实现远距离极化激元凝聚体的相位锁定


<details>
  <summary>Details</summary>
Motivation: 研究空间分离的极化激元凝聚体在相干泵浦下的同步机制，特别是在凝聚体距离足够远、通过渐近尾迹相互作用可忽略的情况下

Method: 数值模拟四波混频过程，分析极化激元与相干驱动相互作用产生自由传播极化激元，并介导凝聚体间相互作用

Result: 四波混频产生的自由传播极化激元能够介导远距离极化激元凝聚体间的相互作用，实现相位锁定

Conclusion: 即使空间分离的极化激元凝聚体无法直接相互作用，通过四波混频产生的自由传播极化激元仍可实现相位同步
...
### [13] [Cathodoluminescence Study of a Quantum Dot in a Nanowire for Single-Photon Emission](https://arxiv.org/abs/2512.09542)
*Francis Granger,Fabrice Donatini,Edith Bellet-Amalric,Kuntheak Kheng,Gilles Nogues,David Ferrand,Joel Cibert*

Main category: physics.optics

TL;DR: 该论文展示了扫描电子显微镜中的阴极发光技术可用于表征纳米线中半导体量子点的单光子发射特性，包括测量相关函数和评估发射纯度。


<details>
  <summary>Details</summary>
Motivation: 开发新型单光子源是量子技术的重要需求，需要高效的表征工具来评估单光子发射器的性能。

Method: 使用扫描电子显微镜中的阴极发光技术，对纳米线中的半导体量子点进行成像、光谱测量和相关函数分析，并通过电子束操控减少寄生发光收集。

Result: 在低温条件下，阴极发光测量的热预算（通过声子边带评估）接近非共振微光致发光的结果，证明该技术能有效表征单光子发射纯度。

Conclusion: 阴极发光技术是寻找新型单光子源的有效工具，不仅能进行成像和光谱分析，还能测量相关函数和表征发射器纯度。
...
### [14] [Conservation of transverse orbital angular momentum for spatiotemporal optical vortices](https://arxiv.org/abs/2512.09615)
*Jordan Adams,Youngbin Park,Andy Chong*

Main category: physics.optics

TL;DR: 研究发现时空光学涡旋的横向轨道角动量在去除窄带和傍轴近似后是守恒的，解决了先前研究中因近似导致的争议


<details>
  <summary>Details</summary>
Motivation: 近年来关于时空光学涡旋横向轨道角动量的研究存在争议，先前基于窄带和傍轴近似的研究得出了非常规结论，甚至需要修改电磁波轨道角动量的定义。本研究旨在澄清这些争议。

Method: 采用电磁波轨道角动量的标准定义，去除窄带和傍轴近似，分析时空光学涡旋的横向轨道角动量在传播过程中的行为。

Result: 研究发现时空光学涡旋的横向轨道角动量对窄带和傍轴近似高度敏感。当去除这些近似后，总横向轨道角动量在传播过程中是守恒的。

Conclusion: 时空光学涡旋的横向轨道角动量守恒问题源于先前研究中采用的近似条件，使用标准定义并去除近似后，横向轨道角动量是守恒的，无需修改电磁波轨道角动量的定义。
...
### [15] [Self-Hybridized Exciton-Polariton Photodetectors From Layered Metal-Organic Chalcogenolates](https://arxiv.org/abs/2512.09691)
*Bongjun Choi,Adam D. Alfieri,Wangleong Chen,Deep Jariwala*

Main category: physics.optics

TL;DR: 二维金属有机硫属化物可实现自混合极化激元光电探测器，无需顶部反射镜，通过厚度调谐多模极化激元工程实现亚带隙光子探测，并显著提升光电性能


<details>
  <summary>Details</summary>
Motivation: 传统激子极化激元需要封闭光学腔实现强耦合，这增加了器件复杂性。研究旨在开发无需顶部反射镜的简化器件结构，同时利用极化激元效应增强光电探测性能

Method: 使用高折射率(~2.5)的二维金属有机硫属化物mithrene (AgSePh)，通过厚度调谐实现多模极化激元工程。利用反射率、光致发光和光电流光谱进行表征，并与理论模型进行定量比较

Result: 实现了亚带隙光子的光电探测，通过陷阱辅助双光子吸收维持强耦合状态。极化激元色散产生超快群速度(~65 μm/ps)，将激子扩散长度从几百纳米扩展到几微米。强耦合器件的光暗电流比提升2.38倍

Conclusion: 二维金属有机硫属化物为极化激元增强的光电探测和光收集提供了实用途径，简化了器件结构，同时显著提升了性能指标
...
### [16] [Constraint-Free Coherent Diffraction Imaging via Physics-Guided Neural Fields](https://arxiv.org/abs/2512.09694)
*Zhe Hu,Zisheng Yao,Yuhe Zhang,Pablo Villanueva-Perez*

Main category: physics.optics

TL;DR: CDIP：一种基于坐标神经场的新型相位恢复框架，无需手工约束即可实现静态和动态CDI重建，显著优于传统算法和深度学习基线。


<details>
  <summary>Details</summary>
Motivation: 相干衍射成像（CDI）虽然能实现原子分辨率成像，但其重建过程（相位恢复）不稳定且不适定，严重依赖手工设计的物体特定约束，限制了该技术的广泛应用。

Method: 提出CDIP框架，结合无监督的坐标基神经场进行静态和动态重建，以及物理一致的前向模型，无需手工约束即可实现稳健的相位恢复。

Result: 在模拟和实验数据集（包括静态样本和动态过程）上评估，CDIP在保真度和稳定性方面显著优于经典迭代算法和深度学习基线方法。

Conclusion: CDIP代表了静态和时间分辨CDI重建的范式转变，为X射线、电子和光学探针等多种相干成像模态（如叠层成像和全息术）提供了广泛适用的框架。
...
### [17] [Structures resistant to Manipulation by all Wavefronts](https://arxiv.org/abs/2512.09696)
*Asher Sabbagh,Michael Horodynski,Rida Khan,Brian Shi,Marin Soljačić*

Main category: physics.optics

TL;DR: 该论文发现存在一类特殊形状的物体，即使采用最优波前整形技术也无法有效操控，其最大拉力可降低三个数量级，最大捕获刚度降低近两个数量级。


<details>
  <summary>Details</summary>
Motivation: 光操控微小粒子在生物物理和纳米技术中有广泛应用，波前整形技术显著提升了操控性能，但现有研究尚未探索是否存在无法被有效操控的物体形状。

Method: 采用逆向设计方法寻找无法被有效操控的物体形状，分析最优波前整形协议下的操控极限。

Result: 发现存在多种反直觉形状的物体，即使采用最优波前整形也无法按需操控，最大拉力降低达三个数量级，最大捕获刚度降低近两个数量级。

Conclusion: 这些发现对于开发需要可预测机械响应的微机器具有重要意义，揭示了光操控技术的固有局限性。
...
### [18] [Mode Control and Dynamic Population Gratings in Quantum-Dot Lasers](https://arxiv.org/abs/2512.09740)
*Xiangpeng Ou,Artem Prokoshin,Hongyan Yu,Xin Yao,Ying Shi,William He,Zhican Zhou,Yating Wan*

Main category: physics.optics

TL;DR: 量子点激光器通过动态粒子数光栅实现单模运行，无需复杂制造工艺，具有高边模抑制比、宽调谐范围和强反馈容忍度


<details>
  <summary>Details</summary>
Motivation: 传统集成半导体激光器的单模运行通常需要复杂的制造工艺（如再生长、刻蚀光栅等），限制了可扩展性。需要一种简单、可扩展的方法来实现稳定的单模激光

Method: 利用量子点的低横向载流子扩散特性，在反向偏置的可饱和吸收器中形成强驻波诱导的载流子光栅（动态粒子数光栅），提供自对准、模式选择反馈。设计了单环和双环Vernier激光器结构

Result: 单环激光器达到46 dB边模抑制比；双环Vernier激光器实现>46 nm调谐范围和最高52.6 dB边模抑制比，连续波工作温度达80°C。在-10.6 dB外部光反馈下仍保持单模，支持32 Gbps无隔离器数据传输

Conclusion: 基于动态粒子数光栅的量子点激光器提供了一种简单、可扩展的途径，实现了可调谐、反馈容忍的片上光源，适用于通信、传感和可重构光子系统
...
### [19] [Noise dynamics in large mode volume Brillouin lasers](https://arxiv.org/abs/2512.09855)
*Andrew J. Shepherd,Daniel J. Blumenthal,Ryan O. Behunin*

Main category: physics.optics

TL;DR: 本文开发了集成大模体积布里渊激光器的耦合模理论，考虑了增益带宽内多个可能激射的腔模，揭示了宽带增益导致非典型布里渊动力学，显著影响激光线宽和噪声特性。


<details>
  <summary>Details</summary>
Motivation: 随着布里渊激光器谐振腔长度增加，多个光学共振模式会出现在布里渊增益带宽内，这大大复杂化了激光动力学，而现有耦合模布里渊激光模型无法描述这种物理现象。考虑到这类激光器有望实现瓦级输出功率和亚毫赫兹线宽，需要理论模型来提供关键性能洞察。

Method: 开发了集成大模体积布里渊激光器的耦合模理论，考虑了增益带宽内多个可能激射的腔模。获得了稳态动力学、自发光谱、相对强度噪声和频率噪声的表达式。

Result: 分析表明，宽带增益导致非典型布里渊动力学，在噪声谱中产生独特特征，从而改变了布里渊激光器标准的单模基本线宽。此外，这些特征可用于声子光谱学或品质因数增强等应用。还发现线宽会受到外部泵浦传递的相对强度噪声的显著影响。

Conclusion: 本文的理论模型为理解大模体积布里渊激光器的复杂动力学提供了关键工具，揭示了宽带增益环境下的非典型行为，对实现高性能布里渊激光器具有重要意义。
...
### [20] [Optical arbitrary waveform generation using spectro-temporal unitary transforms](https://arxiv.org/abs/2512.09919)
*Callum Deakin*
...
<details>
  <summary>Details</summary>
Motivation: 传统光学波形生成方法受到调制器带宽限制，需要一种能够实现任意波形生成且不受带宽约束的新方法

Method: 采用级联相位调制器和色散元件组合，通过相位调制和色散效应的协同作用实现光学波形变换

Result: 该方法理论上能够实现任意光学波形生成，不受调制器带宽限制，且保持理论上的无损耗特性

Conclusion: 级联相位调制器和色散元件组合为任意光学波形生成提供了一种有前景的方法，突破了传统带宽限制
